...
def scan_lua_code_mock_fixture() -> Iterator[Mock]:
    with patch("dcs.liveries.liveryscanner.LiveryScanner.scan_lua_code") as mock:
        yield mock
...
def test_scanning_livery_adds_one_livery() -> None:
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code("", "foobar", "unit")

    assert len(scanner.map.keys()) == 1
    assert len(scanner.map["unit"]) == 1


def test_placeholder_livery() -> None:
    """Tests that liveries named "placeholder" are ignored."""
    scanner = LiveryScanner()
    scanner.scan_lua_code("", "placeholder", "unit")
    scanner.scan_lua_code("", "placeholder.zip", "unit")

    assert len(scanner.map.keys()) == 0
...
def test_named_livery() -> None:
    ...
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code('name = "Foobar"', "foobar", "unit")

    livery = scanner.map["unit"].pop()
...
def test_livery_defaults() -> None:
    ...
    livery: Livery = scanner.map["unit"].pop()
    assert livery.name == "foobar"
...
def test_livery_id_from_directory() -> None:
    ...
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code("", str(Path("foo/bar")), "unit")
...
def test_livery_id_from_zip() -> None:
    ...
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code("", str(Path("foo/bar.zip")), "unit")
...
def test_parse_single_country() -> None:
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code('countries = {"USA"}', "foobar", "unit")
...
def test_parse_multiple_countries() -> None:
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code('countries = {"USA", "RUS"}', "foobar", "unit")
...
def test_sort_order() -> None:
    scanner = LiveryScanner()
    scanner.map["unit"] = LiverySet()
    scanner.scan_lua_code("order = 2", "foobar", "unit")
...
def test_ansi_encoded_description(tmp_path: Path, scan_lua_code_mock: Mock) -> None:
    ...
    LiveryScanner().scan_lua_description(str(tmp_path), "foobar")
